...
f = lambda x : np.array([0.5*np.sin(2*np.pi*freq*x),0.1*np.sin(2*np.pi*freq*x)])
u = np.array([f(t[-1])])
a = np.array([integrator(model, x[-1,:], u[-1])[1]])
...
ax.plot(t, a) # velocity
ax.set_ylabel("Acceleration")
ax = fig.add_subplot(513)
ax.plot(t, x[:,2:]*180/np.pi) # velocity
ax.set_ylabel("Velocity")
ax = fig.add_subplot(514)
ax.plot(t, x[:,:2]*180/np.pi) #Theta
ax.set_ylabel("Theta (deg)")

ax = fig.add_subplot(515)
ax.plot(x[:,0]*180/np.pi,x[:,1]*180/np.pi) #Theta
